...
def _to_fetched(item: NewsRssEvent) -> FetchedEvent:
    return FetchedEvent(
        title=item.title,
        ...
        *,
        ...
        for item in items:
            try:
                ...
                logger.exception("PLN import failed for %s", item.title)
