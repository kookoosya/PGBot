...
def build_ai_plans() -> list[AIPlan]:
    return [
        AIPlan(
            ...
            features=(
                f"{settings.AI_FREE_DAILY_LIMIT} сообщений или генераций картинок в день",
                "Базовый ChatGPT (быстрый режим)",
                "Без памяти между днями",
            ),
            chat_modes=("chat",),
        ...
        AIPlan(
            ...
            tagline="Для учёбы, кода и постоянной работы",
            features=(
                f"До {settings.AI_PRO_DAILY_LIMIT} сообщений в день",
                "Умный ChatGPT (GPT-4o)",
                "Режим «Учёба» — курсовые, рефераты, проверка текстов",
                "Режим «Код» — помощь с программированием",
                "Генерация картинок в общем лимите",
            ),
        ...
        AIPlan(
            ...
            period_days=settings.AI_PRO_PERIOD_DAYS,
            tagline="Максимум для активных пользователей",
            features=(
                f"До {settings.AI_PRO_PLUS_DAILY_LIMIT} сообщений в день",
                "Приоритетный доступ к умной модели",
                "Все режимы: чат, учёба, код",
