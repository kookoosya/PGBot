"""Shared RSS parser for regional news sites (Informpskov, PLN, etc.)."""
...
_INVITE_SIGNAL_RE = re.compile(
    r"приглашает|приглашаем|пройд[её]т|откроется|состоится|начн[её]тся",
    re.IGNORECASE,
)
_PAST_RE = re.compile(
...
def _strip_html(value: str) -> str:
    text = html.unescape(re.sub(r"<[^>]+>", " ", _unwrap_cdata(value)))
...
def _parse_rss_items(xml_text: str) -> list[dict[str, str]]:
    ...
    for item in channel.findall("item"):
        description = _unwrap_cdata(item.findtext("description") or "")
        items.append({
            "title": _unwrap_cdata((item.findtext("title") or "").strip()),
            "link": (item.findtext("link") or "").strip(),
            "description": description,
            "category": _unwrap_cdata((item.findtext("category") or "").strip()),
            "pub_date": item.findtext("pubDate") or "",
        })
    return items


def parse_news_rss(xml_text: str, *, fallback_url: str, now: datetime | None = None) -> list[NewsRssEvent]:
    """Parse regional news RSS into normalized events."""
    ...
    for item in _parse_rss_items(xml_text):
        ...
        link = item["link"] or fallback_url
        if link in seen:
            continue
        seen.add(link)
...
async def fetch_news_rss_events(url: str, *, item_limit: int = 40) -> list[NewsRssEvent]:
    ...
    try:
        async with httpx.AsyncClient(timeout=25.0, follow_redirects=True, headers=headers) as client:
            ...
            events = parse_news_rss(response.text, fallback_url=url)
            return events[:item_limit]
    except Exception:
        logger.exception("News RSS fetch failed for %s", url)
        return []
