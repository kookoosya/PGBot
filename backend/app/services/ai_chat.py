...
from app.services.pushkin_quotes import maybe_append_verified_quote
...
- культура, история, маршруты по Пушкиногорью (Михайловское, Тригорское, Свято-Успенский Святогорский монастырь)
...
- Не выдумывай цитаты Пушкина — используй только проверенные тексты с указанием произведения
"""
...
def _finalize_reply(message: str, text: str) -> str:
    return maybe_append_verified_quote(message, text)
...
async def chat_with_ai(message: str, history: list[dict] | None = None, model_id: str | None = None) -> str:
    ...
    if is_valid_pollinations_key(settings.POLLINATIONS_API_KEY):
        ...
        if poll_text:
            return _finalize_reply(message, poll_text)
