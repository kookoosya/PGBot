...
async def visit_stats(
    ...
    async def count_since(since: datetime) -> int:
        return (await db.execute(select(func.count(PageVisit.id)).where(PageVisit.visited_at >= since))).scalar() or 0

    async def unique_since(since: datetime) -> int:
        return (
            await db.execute(
                select(func.count(func.distinct(PageVisit.visitor_key))).where(
                    PageVisit.visited_at >= since, PageVisit.visitor_key.isnot(None)
                )
            )
        ).scalar() or 0
    ...
    top_pages = [
        PageStat(path=path, label=PAGE_LABELS.get(path, path), count=count) for path, count in top_result.all()
    ]
