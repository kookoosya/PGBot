...
async def list_taxi(db: Annotated[AsyncSession, Depends(get_db)]):
    taxi = await list_active_taxi(db)
    return [TaxiServiceResponse.model_validate(item) for item in taxi]


@router.get("/map/stats", response_model=MapStatsResponse)
async def map_stats(db: Annotated[AsyncSession, Depends(get_db)]):
    return (await get_map_stats(db)).to_response()
...
async def list_places(
    ...
    result = await search_places(
        ...
        PlaceSearchParams(
            ...
            page=page,
            page_size=page_size,
            sort_by=sort if sort in {"rating", "name"} else "rating",
            district=district,
        ),
    )
...
async def add_review(
    ...
    try:
        result = await add_place_review(
            db,
            place_id,
            to_review_input(data),
            user=current_user,
        )
    except PlaceNotFoundError as exc:
        raise_http_for_service_error(exc)
    return review_to_response(result.review)
...
async def sync_osm(
    ...
    return await sync_places_from_osm(db)
