...
from app.models.enums import ClassifiedCategory, ClassifiedPaymentStatus, EventRegion
from app.services.classified.schemas import ClassifiedActorContext, ClassifiedCreateInput, ClassifiedSearchParams
from app.services.classified_service import create_classified_ad, moderate_classified_ad, search_classifieds
from tests.helpers.db_factories import create_event, create_owner_user, create_user
...
async def test_public_info_has_contact_links(api_client: AsyncClient):
    ...
    assert response.status_code == 200
    data = response.json()
    for key in ("site_url", "vk_url", "map_url", "vk_bot_ready", "vk_bot_hint"):
        assert key in data
